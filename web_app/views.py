from django.shortcuts import render, redirect
from . models import info_one, info_two
import shutil



# import file for text start here
from django.http import HttpResponse
# import file for text end here


# import file for pdf start here
from django.http import FileResponse
import io
from reportlab.pdfgen import canvas
from reportlab.lib.units import inch
from reportlab.lib.pagesizes import letter
# import file for pdf end here

# import file for csv start here
import csv
# import file for csv end here


# Create your views here.

def index(request):
    info_one_t = info_one.objects.all()
    info_two_t = info_two.objects.all()
    return render(request, 'index.html', {'info_one_t':info_one_t, 'info_two_t':info_two_t})

def save_info(request):
    name = request.POST.get('name')
    roll = request.POST.get('roll')
    k = info_one(name = name, roll = roll)
    k.save()
    return redirect('index')

def move_value(request, pk):
    print(pk)
    k = info_one.objects.get(id = pk)
    print(k.name)
    j = info_two(name = k.name, roll=k.roll)
    j.save()
    k.delete()
    return redirect('index')

def back_value(request, pk):
    k = info_two.objects.get(id=pk)
    print(k.name)
    j = info_one(name=k.name, roll=k.roll)
    j.save()
    k.delete()
    return redirect('index')

from reportlab.lib.colors import Color, black, blue, red
from reportlab.graphics.shapes import Rect
from reportlab.platypus import SimpleDocTemplate
from reportlab.platypus.tables import Table
from reportlab.platypus import Paragraph, Table, TableStyle
from reportlab.lib.enums import TA_JUSTIFY, TA_LEFT, TA_CENTER
from reportlab.lib import colors

def pdf_file(request):
    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=letter, bottomup=0)
    textob = c.beginText()
    textob.setTextOrigin (inch, inch)
    textob.setFont("Helvetica", 14)
    info_one_t = info_one.objects.all()
    f='ffff'
    e='ffff'
    lines = [f'{f}  {e} hello']
    rr  = c.drawString(300, 300, "Hello world.")


    imag = 'data:image/jpeg;base64,'
    c.drawImage(imag, 200, 200, width=72, height=72)

    # textob['Content-Disposition'] = 'PDF'
    #if i want a plan text then
    # lines=[
    #     "this is ok",
    #     "this is ok",
    #     "this is ok",
    # ]

    for i in info_one_t:
        lines.append(i.name)
        lines.append(i.roll)

    for line in lines:
        textob.textLine(line)

    c.drawText(textob)
    red50transparent = Color(100, 0, 0, alpha=0.5)
    c.setFillColor(blue)
    c.setFont('Helvetica', 10)
    c.drawString(25, 180, 'solid')
    c.setFillColor(blue)
    c.rect(25, 25, 100, 100, fill=True, stroke=False)
    c.setFillColor(red)
    c.rect(100, 75, 100, 100, fill=True, stroke=False)
    c.setFillColor(red)
    c.drawString(225, 180, 'transparent')
    c.setFillColor(blue)
    c.rect(225, 25, 100, 100, fill=True, stroke=False)
    c.setFillColor(red50transparent)
    c.rect(300, 75, 100, 100, fill=True, stroke=False)
    for i in info_one_t:
        lines.append(i.name+' '+i.roll)
        lines.append(i.roll)

    for line in lines:
        textob.textLine(line)
    c.drawText(textob)
    c.line(0, 50, 600, 50)


    c.setFillColor(red50transparent)
    c.rect(70, 75, 700, 100, fill=True, stroke=False)


    c.showPage()
    c.save()
    buf.seek(0)
    pdf_file_ooo()
    return FileResponse(buf, as_attachment=True, filename='p.pdf')


def pdf_file_ooo():
    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=letter, bottomup=0)
    textob = c.beginText()
    textob.setTextOrigin(inch, inch)
    textob.setFont("Helvetica", 14)
    info_one_t = info_one.objects.all()
    f = 'ffff'
    e = 'ffff'
    lines = [f'{f}  {e} hello']
    rr = c.drawString(300, 300, "Hello world.")

    imag = 'data:image/jpeg;base64,'
    c.drawImage(imag, 200, 200, width=72, height=72)

    # textob['Content-Disposition'] = 'PDF'
    # if i want a plan text then
    # lines=[
    #     "this is ok",
    #     "this is ok",
    #     "this is ok",
    # ]

    for i in info_one_t:
        lines.append(i.name)
        lines.append(i.roll)

    for line in lines:
        textob.textLine(line)

    c.drawText(textob)
    red50transparent = Color(100, 0, 0, alpha=0.5)
    c.setFillColor(blue)
    c.setFont('Helvetica', 10)
    c.drawString(25, 180, 'solid')
    c.setFillColor(blue)
    c.rect(25, 25, 100, 100, fill=True, stroke=False)
    c.setFillColor(red)
    c.rect(100, 75, 100, 100, fill=True, stroke=False)
    c.setFillColor(red)
    c.drawString(225, 180, 'transparent')
    c.setFillColor(blue)
    c.rect(225, 25, 100, 100, fill=True, stroke=False)
    c.setFillColor(red50transparent)
    c.rect(300, 75, 100, 100, fill=True, stroke=False)
    for i in info_one_t:
        lines.append(i.name + ' ' + i.roll)
        lines.append(i.roll)

    for line in lines:
        textob.textLine(line)
    c.drawText(textob)
    c.line(0, 50, 600, 50)

    c.setFillColor(red50transparent)
    c.rect(70, 75, 700, 100, fill=True, stroke=False)

    c.showPage()
    c.save()
    buf.seek(0)
    return FileResponse(buf, as_attachment=True, filename='p2.pdf')






def csv_file(request):
    response  = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename = p.csv'

    writer = csv.writer(response)
    writer.writerow(['Name', 'Roll'])

    info_one_t = info_one.objects.all()

    for i in info_one_t:
        writer.writerow([i.name, i.roll])
    return response


def txt_file(request):
    response = HttpResponse(content_type='text/plain')
    response['Content-Disposition'] = 'attachment; filename = p.txt'

    info_one_t = info_one.objects.all()
    lines = []

    #if i want a plan text then
    # lines=[
    #     "this is ok",
    #     "this is ok",
    #     "this is ok",
    # ]

    for i in info_one_t:
        lines.append(f'{i.name} {i.roll}\n')
        # ae vabao hoba
        # lines.append(i.name)
        # lines.append(i.roll)

    response.writelines(lines)
    return response


#test for json start here

from django.core import serializers
from django.http import JsonResponse

def json_def(request):

    info_one_t = info_one.objects.all()
    info_two_t = info_two.objects.all()

    get_cat_seri_info_one_t = serializers.serialize('json', info_one_t)
    get_cat_seri_info_two_t = serializers.serialize('json', info_two_t)

    return render(request, 'json_page.html', {'get_cat_seri_info_one_t': get_cat_seri_info_one_t, 'get_cat_seri_info_two_t': get_cat_seri_info_two_t})


import http.client as ht



def click_to_send(request):

    conn = ht.HTTPConnection("https://smsplus.sslwireless.com/api/v3/send-sms")

    payload = '''{
     "api_token": "744d2817-6c3b-4a70-a91e-e3f9ee5cf1b5",
     "sid": "BOOMBOOMNONAPI",
     "msisdn": "8801781445825",
     "sms": "হ্যালো",
     "csms_id": "4473433434pZ684333392"
    }'''

    headers = {'authkey': "",
               'content-type': "application/json"}

    conn.request("GET", "https://smsplus.sslwireless.com/api/v3/send-sms?api_token=744d2817-6c3b-4a70-a91e-e3f9ee5cf1b5&sid=BOOMBOOMNONAPI&sms=হ্যালো&msisdn=8801781445825&csms_id=4473433434pZ684333392", payload, headers)

    res = conn.getresponse()

    return render(request, 'json_page.html')
